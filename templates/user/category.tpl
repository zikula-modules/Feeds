...
{if $enablecategorization}
    ...
    <div class="feed-title">
        {if $shorturls and $shorturlstype eq 0}
            <a href="{modurl modname='Feeds' func='view' prop=$property cat=$category.path|replace:$rootCat.path:''}" title="{gt text='Category: %s' tag1=$categoryname}">{gt text='Category: %s' tag1=$categoryname}</a>
        {else}
            <a href="{modurl modname='Feeds' func='view' prop=$property cat=$category.id}" title="{gt text='Category: %s' tag1=$categoryname}">{gt text='Category: %s' tag1=$categoryname}</a>
        {/if}
    </div>
    ...
    <div class="feed-list">
        ...
        <div class="feeditem">
            ...
            <div class="feeditem-info">
                ...
                &nbsp;-&nbsp;
                <a href="{modurl modname='Feeds' func='display' fid=$FeedLinkInfo.fid}" title="{$FeedLinkInfo.name|safetext}">{$FeedLinkInfo.name|safetext}</a>
                &nbsp;
    ...
    <div class="feed-bottom">
        ...
        {modurl modname='Feeds' func='category' startnum=$startnum assign='returnurl'}
    </div>
{else}
